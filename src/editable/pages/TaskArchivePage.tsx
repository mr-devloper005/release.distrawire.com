  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#eef6ff] text-[#081426]">
        <section className="relative overflow-hidden bg-[linear-gradient(115deg,#3486db_0%,#14a7c4_48%,#10c6ad_100%)] text-white">
          <div className="absolute -right-24 -top-24 h-96 w-96 rounded-full bg-white/10 blur-3xl" />
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-8 px-4 py-14 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-20">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-white/75">Newswire newsroom</p>
              <h1 className="mt-4 text-5xl font-light leading-tight tracking-[-0.03em] sm:text-6xl lg:text-7xl">
                Latest {category === 'all' ? label : categoryLabel}
              </h1>
              <p className="mt-5 max-w-2xl text-xl font-black leading-8">Press releases, media updates, and distribution-ready announcements in one searchable newsroom.</p>
            </div>
            <form action={basePath} className="flex w-full max-w-md rounded-[5px] bg-white p-2 text-[#081426] shadow-xl">
              <select name="category" defaultValue={category} className="min-w-0 flex-1 bg-transparent px-3 text-sm font-black outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="rounded-[5px] bg-[#235aaa] px-5 text-sm font-black text-white">Filter</button>
            </form>
          </div>
        </section>

        <section className="border-b border-[#d6e4f2] bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-5 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#2457ae] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'rounded-full bg-[#d9fff7] px-4 py-2 text-[#0aa58f]' : 'rounded-full px-4 py-2 hover:bg-[#eef6ff]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'whitespace-nowrap rounded-full bg-[#d9fff7] px-4 py-2 text-[#0aa58f]' : 'whitespace-nowrap rounded-full px-4 py-2 hover:bg-[#eef6ff]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] gap-6 px-4 py-12 sm:px-6 lg:grid-cols-[1.2fr_.8fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[16px] bg-[#285aa8] shadow-[0_24px_70px_rgba(28,76,130,.18)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[#10c6ad] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-[1.04] tracking-[-0.035em] sm:text-5xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
                <span className="mt-6 inline-flex items-center gap-3 text-lg font-medium">Read PR <ArrowRight className="h-4 w-4" /></span>
              </div>
            </Link>
            <div className="grid gap-5">
              <div className="rounded-[14px] bg-white p-6 shadow-sm">
                <p className="text-xs font-black uppercase tracking-[0.24em] text-[#2f6ee5]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">Fresh announcements moving through the wire.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[8rem_1fr] overflow-hidden rounded-[14px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                  <img src={getImage(post)} alt="" className="h-full min-h-36 w-full object-cover transition group-hover:scale-105" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#2f6ee5]">0{index + 1}</p>
                    <h3 className="mt-3 text-lg font-black leading-tight">{post.title}</h3>
                    <span className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-[#0b55b6]">Read PR <ArrowRight className="h-4 w-4" /></span>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
            <h2 className="text-4xl font-black tracking-[-0.03em] sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex rounded-[5px] bg-white shadow-sm">
              ...
              <button className="h-11 rounded-[5px] bg-[#10c6ad] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>

          {remaining.length ? (
            <div className="columns-1 gap-10 space-y-10 md:columns-2 xl:columns-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group mb-10 block break-inside-avoid border-b border-[#d6e4f2] pb-7">
                  {index % 3 !== 1 ? <img src={getImage(post)} alt="" className="aspect-[16/10] w-full rounded-[12px] object-cover transition duration-500 group-hover:scale-[1.02]" /> : null}
                  <p className="mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-[#2f6ee5]">{getCategory(post, label)}</p>
                  <h3 className="mt-3 text-xl font-black leading-[1.25]">{post.title}</h3>
                  <p className="mt-4 line-clamp-3 text-sm leading-6 text-[#3c526e]">{getSummary(post)}</p>
            ...
            <div className="rounded-[12px] border border-dashed border-[#bdd2eb] bg-white p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-black">No releases found</h2>
              <p className="mt-2 text-sm text-[#3c526e]">Try another category or publish a new newsroom story.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-[5px] border border-[#bdd2eb] bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="bg-[#235aaa] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-[5px] border border-[#bdd2eb] bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
