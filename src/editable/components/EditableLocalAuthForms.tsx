...
const inputClass = 'h-[3.25rem] rounded-[10px] border border-[#d6e4f2] bg-[#f7fbff] px-4 text-base font-bold text-[#081426] outline-none transition placeholder:text-[#6c7d95] focus:border-[#2f6ee5] focus:bg-white'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-[5px] bg-[#10c6ad] px-6 text-sm font-black text-white transition hover:bg-[#0aa58f] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[10px] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}>{message}</p> : null}
